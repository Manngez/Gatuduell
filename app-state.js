...
(() => {
  ...
  const S={requested:store.get('city','umea'),city:null,profile:null,session:null,graph:null,map:null,highlight:null,timer:null,state:null,modalMode:'setup',mapExplore:false,lastShare:null};
  ...
  async function loadCity(c=S.city){
    ...
    try{
      ...
      if(S.highlight){S.highlight.remove();S.highlight=null}S.map?.setView(S.city.center,S.city.zoom||13,{animate:false});S.map?.invalidateSize(false);
  ...
  window.GatduellApp={E,C,B,$,S,LEVELS,TARGET:3,store,escapeHtml,premium,locked,level,open,close,live,initMap,setMapExplore,cityUI,loadCity,restore};
})();
