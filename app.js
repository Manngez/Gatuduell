...
(() => {
  ...
  const LEVELS={hard:{label:'Svår',steps:1,icon:'🔴'},medium:{label:'Medel',steps:2,icon:'🟡'},easy:{label:'Lätt',steps:3,icon:'🟢'}};
  ...
  async function loadGraph(){
    if(loadGraph.running)return;
    loadGraph.running=true;$('retryLoadBtn').hidden=true;
    $('loadStatus').textContent='Laddar gatunätet…';
    const activate=(data,cached=false)=>{
      const features=data?.features||[];
      if(features.length<100)throw new Error('För få vägsegment');
      const candidate=E.buildGraph(features,{bbox:E.DEFAULT_BBOX,toleranceMeters:7,junctionRadiusMeters:32});
      if(candidate.size<100)throw new Error('För få spelbara gator');
      graph=candidate;
      $('loadStatus').textContent=`${cached?'Sparat gatunät':'Klart'} · ${graph.size.toLocaleString('sv-SE')} spelbara gatunamn`;
      $('startBtn').disabled=false;$('startBtn').textContent='Spela lokalt';$('onlineBtn').disabled=false;
    };
    try{
      // A packaged snapshot also lets a first-time visitor play during a data outage.
      for(const url of ['roads-fallback.json.gz',...DATA_URLS]){
        ...
        try{
          ...
          const data=url.endsWith('.gz')?await new Response(res.body.pipeThrough(new DecompressionStream('gzip'))).json():await res.json();activate(data,url==='roads-fallback.json.gz');
          try{localStorage.setItem('gatuduell:roads:v1',JSON.stringify(data))}catch{}
          return;
        }catch{}finally{clearTimeout(timeout)}
      }
      try{activate(JSON.parse(localStorage.getItem('gatuduell:roads:v1')),true);return}catch{}
      $('loadStatus').textContent='Gatunätet kunde inte hämtas. Kontrollera anslutningen och försök igen.';
      $('startBtn').textContent='Gatunät saknas';$('retryLoadBtn').hidden=false;
    }finally{loadGraph.running=false}
  }
  ...
  function initMap(){
    ...
    const tiles=L.tileLayer('https://basemaps.cartocdn.com/light_nolabels/{z}/{x}/{y}.png?key=cb1_2pc7_1_75dfa0fd7c63ee3f904d6d5d',{
      tileSize:256,detectRetina:false,maxZoom:20,
      attribution:'&copy; OpenStreetMap &copy; CARTO'
    }).addTo(map);
    let tileErrors=0,fallback=null;
    tiles.on('tileerror',()=>{
      if(++tileErrors<3||fallback||!graph)return;
      fallback=L.layerGroup().addTo(map);
      for(const street of graph.streets.values())for(const coords of street.lines){
        L.polyline(coords.map(([lon,lat])=>[lat,lon]),{color:'#bec9ce',weight:2,interactive:false}).addTo(fallback);
      }
    });
  ...
  function markUserNavigation(){if(mapProgrammatic)return;userNavigatingUntil=Date.now()+FREE_NAV_MS;}
  ...
  function focusCurrent(force=false){
    ...
    withProgrammaticMap(()=>map.flyToBounds(L.latLngBounds(points),{paddingTopLeft:[20,180],paddingBottomRight:[20,180],maxZoom:16,animate:false}));
    $('mapHint').textContent='Röd: aktuell gata · Grå: redan använd';
  }
  ...
  function makeState(players,settings={}){return {roomId:net.room||'LOCAL',revision:0,status:'LOBBY',hostId:net.playerId||'local',gameMasterId:net.playerId||'local',players,playerOrder:players.filter(p=>p.active).map(p=>p.id),turnId:0,pauseReason:null,remainingMs:null,currentPlayer:null,currentStreet:null,usedStreets:[],scores:Object.fromEntries(players.map(p=>[p.id,0])),round:1,difficulty:settings.difficulty||$('difficulty').value,timeLimit:Number(settings.timeLimit??$('turnSeconds').value)||0,deadline:0,winner:null,gameStatus:'LOBBY',result:null,lastAnswer:null,updatedAt:Date.now()}}
  function bump(){if(state){state.revision=(state.revision||0)+1;state.updatedAt=Date.now()}}
  function activePlayers(){return (state?.players||[]).filter(p=>p.active&&p.approved&&p.connected)}

  function currentIndex(){return Math.max(0,state.playerOrder.indexOf(state.currentPlayer))}
  function nextPlayer(from=state.currentPlayer){
    const order=state.playerOrder;const start=order.indexOf(from);
    for(let step=1;step<=order.length;step++){
      const id=order[(start+step+order.length)%order.length];
      if(activePlayers().some(p=>p.id===id))return id;
    }
    return null;
  }
  function pauseGame(reason){
    if(state?.gameStatus!=='PLAYING')return;
    state.remainingMs=state.timeLimit?Math.max(0,state.deadline-Date.now()):null;
    stopTimer();state.deadline=0;state.pauseReason=reason;state.status=state.gameStatus='PAUSED';
    bump();render(false);broadcastState();
  }
  function reconcilePlayers(){
    if(net.role!=='host'||!state)return;
    if(state.gameStatus==='PLAYING'&&activePlayers().length<2){pauseGame('connection');return;}
    const current=activePlayers().some(p=>p.id===state.currentPlayer);
    if(state.gameStatus==='PLAYING'&&!current){state.currentPlayer=nextPlayer();beginTurn();return;}
    if(state.gameStatus==='PAUSED'&&state.pauseReason==='connection'&&activePlayers().length>=2){
      state.status=state.gameStatus='PLAYING';state.pauseReason=null;
      if(!current){state.currentPlayer=nextPlayer();beginTurn();}
      else{startTurnTimer(state.remainingMs);bump();render(false);broadcastState();}
    }
  }
  function disconnectPlayer(id,conn){
    if(net.role!=='host'||net.connections.get(id)!==conn)return;
    net.connections.delete(id);
    const player=state?.players.find(p=>p.id===id);
    if(player){player.connected=false;bump();reconcilePlayers();render(false);broadcastState();}
  }
  ...
  function beginRound(){
    stopTimer();if(activePlayers().length<2){setMessage('Väntar på minst två anslutna spelare.');return;}state.currentStreet=E.chooseStart(graph);state.usedStreets=[state.currentStreet];const order=state.playerOrder.filter(id=>activePlayers().some(p=>p.id===id));state.currentPlayer=order[(state.round-1)%order.length]||null;state.status=state.gameStatus='PLAYING';state.pauseReason=null;state.remainingMs=null;state.result=null;state.deadline=0;bump();
    $('pathHint').hidden=true;setMessage('Skriv en gata som ligger rätt från den röda gatan.');render(true);beginTurn();
  }
  function beginTurn(){
    if(state?.gameStatus!=='PLAYING')return;
    if(activePlayers().length<2){pauseGame('connection');return;}
  ...
  function setTurnInput(){
    const canPlay=state?.gameStatus==='PLAYING'&&(net.role==='offline'||state.currentPlayer===net.playerId)&&(net.role!=='guest'||net.conn?.open);
    $('streetInput').disabled=!canPlay||net.pendingMove;$('submitBtn').disabled=!canPlay||net.pendingMove;
    if(state?.gameStatus==='PAUSED')setMessage(state.pauseReason==='connection'?'Pausat · väntar på minst två anslutna spelare.':'Spelet är pausat.');
  ...
  function submitMove(){
    ...
    if(net.role==='guest'){if(state.currentPlayer!==net.playerId||net.pendingMove)return;if(!net.conn?.open){setMessage('Återansluter. Försök igen när anslutningen är tillbaka.','bad');return;}net.pendingMove=true;net.pendingPacket={type:'move',raw,playerId:net.playerId,moveId:crypto.randomUUID(),turnId:state.turnId,revision:state.revision};setTurnInput();setMessage('Skickar svaret till spelledaren…');sendHost(net.pendingPacket);armMoveRetry();return;}
    if(net.role==='host'&&state.currentPlayer!==net.playerId)return;
    processMove(raw);
  }
  function processMove(raw){
    if(state?.gameStatus!=='PLAYING')return;
    if(state.timeLimit&&state.deadline&&Date.now()>=state.deadline)return finishRound('Tiden tog slut.',nextPlayer(),true);
    stopTimer();const player=state.players.find(p=>p.id===state.currentPlayer);
  ...
  function showResult(){
    ...
    $('resultBtn').disabled=net.role==='guest';$('resultModal').hidden=false;
  }
  ...
  function startTurnTimer(remainingMs=null){
    ...
    state.deadline=Date.now()+(remainingMs??seconds*1000);state.remainingMs=null;$('timerTrack').hidden=false;
    const tick=()=>{
      if(state?.gameStatus!=='PLAYING'){stopTimer();return;}
      const left=Math.max(0,state.deadline-Date.now());paintTimer(left,seconds);
      if(left<=0){stopTimer();if(net.role!=='guest')finishRound('Tiden tog slut.',nextPlayer(),true);}
    };
    tick();if(state.gameStatus==='PLAYING')timer=setInterval(tick,100);
  }
  ...
  function leaveNetwork(closeModal=true){stopTimer();stopDisplayTimer();closePeer();net.room='';net.name='';net.status='offline';net.playerId='';state=null;if(closeModal)closeOnlineModal();renderOnlineStatus()}
  ...
  function applyRemoteState(remote){
    if(!remote||remote.roomId!==net.room||(state&&remote.revision<=state.revision))return;
    const oldTurn=state?.turnId,oldRound=state?.round;state=clone(remote);net.hostPlayerId=state.hostId;
    if(net.pendingPacket&&state.turnId!==net.pendingPacket.turnId)clearPendingMove();
    if(state.gameStatus==='CLOSED'){backToSetup();return;}
    if(state.gameStatus!=='LOBBY'&&state.gameStatus!=='STARTING'&&$('gameScreen').hidden)enterGuestGame(state);
    else if(!$('gameScreen').hidden){render(oldRound!==state.round);setTurnInput();}
    if(oldTurn!==state.turnId){$('streetInput').value='';$('suggestions').innerHTML='';}
    const path=state.lastAnswer?.result;
    $('pathHint').hidden=!(path?.ok&&path.steps>1);
    if(!$('pathHint').hidden)$('pathHint').textContent=`✓ ${path.steps} steg: ${path.path.join(' → ')}`;
    if((state.gameStatus==='ROUND_END'||state.gameStatus==='GAME_OVER')&&state.result)showResult();else $('resultModal').hidden=true;
    renderOnlineStatus();startGuestDisplayTimer();
  }
  function clearPendingMove(){clearTimeout(net.moveTimeout);net.pendingMove=false;net.pendingPacket=null;}
  function armMoveRetry(){
    clearTimeout(net.moveTimeout);
    net.moveTimeout=setTimeout(()=>{
      if(net.role!=='guest'||!net.pendingPacket)return;
      if(net.conn?.open){sendHost(net.pendingPacket);armMoveRetry();}
      else{clearPendingMove();setTurnInput();setMessage('Anslutningen bröts. Skriv svaret igen när du är ansluten.','bad');}
    },2500);
  }
  ...
  function attachHostConnection(conn){
    ...
    conn.on('data',msg=>{
      ...
      if(msg?.type==='hello'){
        if(id)return;
        id=String(msg.playerId||'').slice(0,80);
        if(!/^[a-zA-Z0-9-]{1,80}$/.test(id)||id===state.hostId){conn.send({type:'reject',reason:'Ogiltig spelaridentitet.'});conn.close();return;}
        const old=state.players.find(p=>p.id===id);
        if(!old&&state.gameStatus!=='LOBBY'){conn.send({type:'reject',reason:'Matchen har redan startat.'});conn.close();return;}
        if(!old?.connected&&state.players.filter(p=>p.connected).length>=10){conn.send({type:'reject',reason:'Rummet är fullt · max 10 deltagare'});conn.close();return;}
        const previous=net.connections.get(id);net.connections.set(id,conn);previous?.close();
        if(old){old.connected=true;old.lastSeen=Date.now();}
        else{const player=makePlayer(id,uniqueName(msg.name),{approved:false,lastSeen:Date.now(),peerId:conn.peer});state.players.push(player);net.pending.set(id,player);}
        bump();reconcilePlayers();conn.send({type:'welcome',playerId:id,state:clone(state)});broadcastState();render(false);return;
      }
      if(!id||net.connections.get(id)!==conn)return;
      if(msg?.type==='pong'){const p=state.players.find(x=>x.id===id);if(p)p.lastSeen=Date.now();}
      if(msg?.type==='move'){
        const key=`${id}:${msg.moveId}`;
        if(net.seenMoves.has(key)){conn.send({type:'move-result',moveId:msg.moveId,ok:true,state:clone(state)});return;}
        const sameTurn=msg.turnId===state.turnId;
        if(state.gameStatus!=='PLAYING'||id!==state.currentPlayer||!sameTurn){
          conn.send({type:'move-result',moveId:msg.moveId,ok:false,reason:'Turen har ändrats eller pausats. Kontrollera aktuell gata och försök igen.',state:clone(state)});return;
        }
        net.seenMoves.add(key);if(net.seenMoves.size>1000)net.seenMoves.delete(net.seenMoves.values().next().value);
        processMove(String(msg.raw||'').slice(0,80));
        conn.send({type:'move-result',moveId:msg.moveId,ok:true,state:clone(state)});
      }
    });
    conn.on('close',()=>disconnectPlayer(id,conn));
    conn.on('error',()=>{disconnectPlayer(id,conn);conn.close();});
  }
  ...
  async function scheduleHostMigration(){
    const generation=net.generation;const candidate=migrationCandidate();
    if(!candidate||candidate.id!==net.playerId){net.reconnect=setTimeout(connectToHost,3000);return;}
    net.status='reconnecting';renderOnlineStatus();
    await new Promise(resolve=>setTimeout(resolve,1400));
    if(net.generation!==generation||net.role!=='guest'||net.conn?.open)return;
    try{
      const PeerCtor=await ensurePeer();
      if(net.generation!==generation)return;
      // Keep the existing guest peer alive until the room identity is acquired.
      const previousPeer=net.peer;const peer=new PeerCtor(roomPeerId(net.room),peerOptions());
      peer.on('open',()=>{
        if(net.generation!==generation||net.role!=='guest'){peer.destroy();return;}
        net.role='host';net.peer=peer;previousPeer?.destroy();net.conn=null;clearPendingMove();stopDisplayTimer();
        for(const p of state.players){p.connected=p.id===net.playerId;p.isGameMaster=p.id===net.playerId;}
        net.hostPlayerId=state.hostId=state.gameMasterId=net.playerId;
        if(state.gameStatus==='PLAYING'){
          state.remainingMs=state.timeLimit?Math.max(0,state.deadline-Date.now()):null;
          state.deadline=0;state.status=state.gameStatus='PAUSED';state.pauseReason='connection';
        }
        bump();net.status='ready';startHeartbeat();render(false);renderOnlineStatus();
        if(state.result)showResult();
        setMessage('Du är ny värd. Väntar på att övriga spelare återansluter.');
      });
      peer.on('connection',attachHostConnection);
      peer.on('error',()=>{peer.destroy();if(net.generation===generation&&net.role==='guest')net.reconnect=setTimeout(connectToHost,2500);});
    }catch{if(net.generation===generation)net.reconnect=setTimeout(connectToHost,2500);}
  }

  function attachGuestConnection(conn){
    conn.on('open',()=>{if(net.conn!==conn||net.role!=='guest')return;net.status='connected';net.lastPong=Date.now();conn.send({type:'hello',name:net.name,playerId:net.playerId,peerId:net.peer.id});renderOnlineStatus();});
    conn.on('data',msg=>{
      ...
      if(msg?.type==='move-result'){
        if(net.pendingPacket?.moveId===msg.moveId)clearPendingMove();
        applyRemoteState(msg.state);setTurnInput();if(!msg.ok)setMessage(msg.reason,'bad');
      }
      if(msg?.type==='reject'||msg?.type==='kick'){
        const reason=msg.reason||'Du har tagits bort från rummet.';backToSetup();openOnlineModal();$('onlineError').textContent=reason;return;
      }
      renderOnlineStatus();
    });
    conn.on('close',()=>{
  ...
  function startHeartbeat(){
    ...
    net.heartbeat=setInterval(()=>{
      if(net.role==='host'){
        for(const [id,conn] of net.connections){
          const player=state?.players.find(p=>p.id===id);
          if(player&&Date.now()-(player.lastSeen||Date.now())>16000){disconnectPlayer(id,conn);conn.close();}
        }
        broadcast({type:'ping',at:Date.now()});
      }else if(net.role==='guest'&&Date.now()-net.lastPong>16000){net.conn?.close();}
    },5000);
  }
  ...
  function kickPlayer(id){
    if(net.role!=='host'||!state)return;
    const conn=net.connections.get(id);conn?.send({type:'kick'});net.connections.delete(id);
    state.players=state.players.filter(p=>p.id!==id);state.playerOrder=state.playerOrder.filter(x=>x!==id);
    conn?.close();bump();reconcilePlayers();render(false);broadcastState();renderOnlineStatus();
  }
  ...
  function hostAction(action){
    if(net.role!=='host'||!state)return;
    if(action==='pause')return pauseGame('manual');
    if(action==='resume'&&state.gameStatus==='PAUSED'){
      if(activePlayers().length<2){setMessage('Väntar på minst två anslutna spelare.');return;}
      state.status=state.gameStatus='PLAYING';state.pauseReason=null;
      if(!activePlayers().some(p=>p.id===state.currentPlayer)){state.currentPlayer=nextPlayer();beginTurn();return;}
      startTurnTimer(state.remainingMs);
    }
    if(action==='skip'&&state.gameStatus==='PLAYING'){state.currentPlayer=nextPlayer();beginTurn();return;}
    if(action==='end-round'&&state.gameStatus==='PLAYING')return finishRound('Spelledaren avslutade rundan.',nextPlayer(),false);
    if(action==='restart'){
      if(activePlayers().length<2){setMessage('Väntar på minst två anslutna spelare.');return;}
      for(const p of state.players)p.score=0;state.scores=Object.fromEntries(state.players.map(p=>[p.id,0]));state.round=1;beginRound();return;
    }
    bump();broadcastState();render(false);
  }

  $('retryLoadBtn').addEventListener('click',()=>void loadGraph());
